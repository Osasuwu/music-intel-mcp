...
- **Structural descriptor** — machine-readable summary of a root's evidence (e.g. for `audio`: `{bpm_band, energy, valence, danceability}` each in `{low,mid,high}`; for `temporal`: `{time_bucket, conditioned_root_id, lift}`; etc.). The source of truth for what a root *is*. Every root has one; downstream consumers read this, not prose.
