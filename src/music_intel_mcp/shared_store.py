...
def spotify_track_uri(track_id: str) -> str:
    """An id carrying our internal ``spotify:<id>`` canonical prefix (see
    :func:`canonical_track_id`) becomes a full ``spotify:track:<id>`` uri; an
    id already in that full uri form is passed through as-is. Shared by
    ...
    #158 AC3: raises on any key that isn't ``spotify:``-prefixed (or already
    a full ``spotify:track:`` uri) -- an mbid/isrc/name-keyed track was never
    resolved to a Spotify id, so silently building a uri from its bare
    suffix would point at the wrong track."""
    if track_id.startswith("spotify:track:"):
        return track_id
    if track_id.startswith("spotify:"):
        return f"spotify:track:{track_id.split(':', 1)[1]}"
    raise ValueError(f"not a spotify-prefixed track id: {track_id!r}")
