    ...
    def lookup(self, artist: str, track: str, mbid: str | None) -> list[TrackTag] | None:
        ...
        for source in self._sources:
            try:
                ...
                last_error = exc
                continue
            # This source answered without raising, so it supersedes any
            # earlier error -- but a *later* source erroring after this must
            # still surface, since a clean miss can't vouch for a source that
            # never got to run. Only the outcome of the last source actually
            # reached (tags, clean miss, or error) decides the final result.
            last_error = None
            if tags:
                return tags
