...
def test_cli_import_spotify_first_ever_import_has_no_before_history(tmp_path, capsys):
    """#93 Risk 3: empty ``before`` (no history.jsonl yet) must not trip the
    stale-history guardrail or otherwise misbehave — there is nothing to supersede."""
    export = tmp_path / "export"
    export.mkdir()
    _write_export(
        export / "Streaming_History_Audio_2022.json",
...
def test_cli_import_spotify_n_plus_1_superset_reimport_is_lossless(tmp_path, capsys):
    """#93 AC: a genuinely overlapping (not identical) re-export — Spotify's actual
    re-ship semantics, where export N+1 is a full-history superset of export N, plus
    new plays — must not lose or duplicate anything via the supersede+dedup path."""
    export_n = tmp_path / "export_n"
    export_n.mkdir()
    row_a = _audio_row("2022-10-21T20:19:07Z", "Maniac", "Flower Face", "trackA")
    row_b = _audio_row("2022-10-22T09:00:00Z", "Second", "Flower Face", "trackB")
    _write_export(export_n / "Streaming_History_Audio_2022.json", [row_a, row_b])

    rc = main(["import-spotify", "--from", str(export_n), "--data-dir", str(tmp_path)])
    assert rc == 0
    after_n = UserStore(root=tmp_path).load_history()
    assert len(after_n) == 2

    # N+1: a later full-history re-export shipping row_a and row_b again (unchanged)
    # plus one genuinely new play — the real-world Spotify re-ship shape.
    export_n1 = tmp_path / "export_n1"
    export_n1.mkdir()
    row_c = _audio_row("2023-01-01T10:00:00Z", "Third", "Flower Face", "trackC")
    _write_export(export_n1 / "Streaming_History_Audio_2022.json", [row_a, row_b, row_c])

    rc = main(["import-spotify", "--from", str(export_n1), "--data-dir", str(tmp_path)])
    assert rc == 0
    after_n1 = UserStore(root=tmp_path).load_history()
    assert len(after_n1) == 3  # no duplicates from row_a/row_b, row_c added
    names = sorted(e.track.name for e in after_n1)
    assert names == ["Maniac", "Second", "Third"]
    assert "WARNING" not in capsys.readouterr().out  # normal superset re-import


def test_cli_import_spotify_warns_when_new_export_is_not_a_superset(tmp_path, capsys):
    """#93 Risk 1 guardrail: if the new export's earliest play is later than an
    existing spotify_extended play, that older play is about to be dropped by
    supersede without being re-covered by the new import — warn, don't block."""
    export_n = tmp_path / "export_n"
    export_n.mkdir()
    old_row = _audio_row("2020-01-01T00:00:00Z", "Old", "Flower Face", "trackOLD")
    _write_export(export_n / "Streaming_History_Audio_2020.json", [old_row])
    main(["import-spotify", "--from", str(export_n), "--data-dir", str(tmp_path)])

    # A "wrong" later re-export whose earliest play is well after the old one —
    # i.e. it does not actually re-cover the history it's about to supersede.
    export_bad = tmp_path / "export_bad"
    export_bad.mkdir()
    new_row = _audio_row("2023-06-01T00:00:00Z", "New", "Flower Face", "trackNEW2")
    _write_export(export_bad / "Streaming_History_Audio_2023.json", [new_row])
